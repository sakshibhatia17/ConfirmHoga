...
`http://127.0.0.1:8000/api/find-jugaad?source=${source}&destination=${destination}`
  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      <section className="glass-panel p-8 rounded-2xl relative overflow-hidden shadow-2xl">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-transparent pointer-events-none"></div>
        <div className="relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr_auto] gap-6 items-end">
            {/* Source */}
            <div className="flex flex-col gap-2">
              <label className="font-mono text-xs text-neutral-400 uppercase tracking-widest">Source Station</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 font-mono text-xs">ADI / NDLS</span>
                <input 
                  value={source}
                  onChange={(e) => setSource(e.target.value.toUpperCase())}
                  className="w-full bg-neutral-950 border-b border-white/5 focus:border-cyan-400 text-neutral-100 pl-24 pr-4 py-3 font-mono text-sm focus:outline-none focus:ring-0 transition-all"
                  type="text"
            ...
            {/* Destination */}
            <div className="flex flex-col gap-2">
              <label className="font-mono text-xs text-neutral-400 uppercase tracking-widest">Destination Station</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 font-mono text-xs">BCT / JP</span>
                <input 
                  value={destination}
                  onChange={(e) => setDestination(e.target.value.toUpperCase())}
                  className="w-full bg-neutral-950 border-b border-white/5 focus:border-cyan-400 text-neutral-100 pl-20 pr-4 py-3 font-mono text-sm focus:outline-none focus:ring-0 transition-all"
                  type="text"
                  placeholder="e.g. BCT"
                />
              </div>
            ...
            <button 
              ...
              disabled={loading}
              className="px-8 py-3.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:shadow-[0_0_15px_rgba(160,95,207,0.4)] text-white font-mono text-sm font-bold rounded-lg transition-all h-[46px] flex items-center justify-center gap-2 group active:scale-95 duration-150"
            >
              {loading ? (
                <>
          ...
          {error && (
            <div className="mt-4 p-3.5 rounded-lg bg-rose-500/10 border border-rose-500/25 text-rose-400 text-xs font-mono">
              {error}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 flex flex-col gap-8">
          <h2 className="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-neutral-100 flex items-center gap-3">
          ...
          <div className="relative pl-8 before:content-[''] before:absolute before:left-3 before:top-4 before:bottom-4 before:w-0.5 before:bg-white/5">
            <AnimatePresence mode="popLayout">
              {result && result.segments.map((seg, idx) => (
                <motion.div 
                  key={seg.id}
                ...
                >
                  ...
                  {/* Glass segment card */}
                  <div className="glass-panel rounded-xl p-6 hover:border-indigo-500/20 transition-all group flex flex-col gap-6 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start flex-wrap gap-4">
                      <div>
                        <h3 className="font-geist text-2xl font-bold text-neutral-100 flex items-center gap-3">
          ...
          <div className="glass-panel p-6 rounded-xl border-cyan-400/20 bg-indigo-950/10 relative overflow-hidden shadow-2xl">
            ...
            {result && (
              <div className="flex flex-col gap-6">
                ...
                <button className="w-full mt-6 py-4 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:shadow-[0_0_20px_rgba(109,40,217,0.4)] text-white font-mono text-sm font-bold uppercase rounded-lg transition-all flex items-center justify-center gap-2">
